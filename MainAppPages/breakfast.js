...
export default class iftari extends React.Component {
    state = {
      ...
      count : 0,  //cartList consists of items added to the cart,
    }
    ...
    componentDidMount(){ //built in function runs when the component gets rendered for the first time
      fetchBreakfastMenu()  // extracts breakFast menu  
      .then( received => {
        if(received[0] == 'Menu not available'){
            this.setState({menuAvailable: received[0]})
          } /*puts a menu not available string to menuAvailable property */
       else{
          this.setState({itemList: received}) /*otherwise we get list of items from fetchBreakfastMenu*/
        }
        }).catch(err => console.log(err)) //error is displayed to terminal 
      }
                 ...
                 <Icon style={{marginLeft: '3%'}} onPress={() => this.props.navigation.openDrawer()} /* on pressing Icon on left of app bar, drawer opens */ 
                  name='md-menu' size={40} />
                   <Title style={styles.titleText}>Menu</Title>
                     <View style={{flexDirection:'row'}}/*With flexDir to row, items are aligned horizontally */>
